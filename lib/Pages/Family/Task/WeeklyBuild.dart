...
class WeeklyBuild extends StatefulWidget {
  List<FamilyTaskData> taskData = [];
  Family family;
  RepeatTasks repeatTaskData;

  WeeklyBuild({this.taskData, this.family, this.repeatTaskData});

  @override
  _WeeklyBuildState createState() => _WeeklyBuildState();
}

class _WeeklyBuildState extends State<WeeklyBuild> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) async {
      ...
      for (RepeatTaskData value in widget.repeatTaskData.data) {
        ...
        if (value.familyTask.repeatType == 1) {
  ...
  Future<void> multipeInsertFamilyPersonTask(
    ...
    if (filtListID.length == 0) {
      ...
      if (result == 200) {
        //   _repeatTaskData.addAll(_selectedTask);
